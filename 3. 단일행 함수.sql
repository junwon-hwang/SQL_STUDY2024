...
-- char는 남은 byte는 공백으로 처리해서 남은 공간을 처리함
-- 비교할때 길이가 바이트가 작은쪽이 큰쪽에 맞춤
-- 문자끝에 뒷공백을 넣어서 비교함 
INSERT INTO char_compare VALUES ('101', 'SQLD', 'SQLD');  -- 같음
INSERT INTO char_compare VALUES ('102', 'SQLD', 'SQLA');
INSERT INTO char_compare VALUES ('103', 'SQLD', '  SQLD'); -- 다름


SELECT * FROM char_compare;

-- select 문법에서 where절은 true => 조회가 됨 , false => 조회가 안됨
SELECT * FROM char_compare
WHERE sn = '102'
    AND char_4 > char_6
;
...
FROM dual;-- 아무 의미 없는 테이블 연산과 함수확인용 , 내장 테이블
...
SELECT 
    ...
    LTRIM('HELLOhello', 'HE')  -- 왼쪽에서 발견된 HE를 제거 , 중간글자는 안됨
FROM dual;
...
SELECT
    ...
    -- 시분초를 생략하면 00:00:00 으로 생략
    TO_DATE('20240101', 'YYYYMMDD'),
    -- 일을 생략하면 01로 셋팅
    TO_DATE('202401', 'YYYYMM'),
    -- CHAR로 셋팅하면 202404까지만 셋팅
